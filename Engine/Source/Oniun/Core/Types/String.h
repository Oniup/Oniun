...
namespace Onu
{
    ...
    public:
        ...
        uint64 Find(const StringView& text) const;
        uint64 FindLast(const StringView& text) const;
    ...
    public:
        ...
        constexpr uint64 Find(const StringView& text) const
        {
        ...
        constexpr uint64 FindLast(const StringView& text) const
        {
            uint64 last = NoPos;
            for (uint64 i = 0; i < m_Length - text.Length(); ++i)
            {
