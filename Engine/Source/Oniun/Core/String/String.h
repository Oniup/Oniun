    ...
    uint64 ReplaceChars(const TChar* search, uint64 searchLength, const TChar* replace, uint64 replaceLength, bool findFirst, uint64 offset = 0)
    {
        ...
        if (index != GlobalVars::NoPos)
        {
            if (searchLength == replaceLength)
                Crt::Copy(m_Data.Ptr() + index, replace, replaceLength * sizeof(TChar));
            else
            {
                uint64 oldLength = m_Length;
                Resize(m_Length - searchLength + replaceLength);
                Crt::Move(m_Data.Ptr() + index + replaceLength, m_Data.Ptr() + index + searchLength, (oldLength - index - searchLength) * sizeof(TChar));
                Crt::Copy(m_Data.Ptr() + index, replace, replaceLength * sizeof(TChar));
                m_Data[m_Length] = '\0';
            }
        }
